...
def do_pc2_aug(pc2_args, pc2):
    # pc2, order: rotation, shift, jitter
    # rotation
    if pc2_args['deg_max']:
        angle2 = pc2_args['degree_range']
    else:
        angle2 = np.random.uniform(-pc2_args['degree_range'],
                                   pc2_args['degree_range'])
    cosval2 = np.cos(angle2)
    sinval2 = np.sin(angle2)
    matrix2 = np.array([[cosval2, 0, sinval2],
                        [0, 1, 0],
                        [-sinval2, 0, cosval2]], dtype=pc2.dtype)
    # shift
    shifts2 = np.random.uniform(-pc2_args['shift_range'],
                                pc2_args['shift_range'],
                                (1, 3)).astype(np.float32)

    pc2[:, :3] = pc2[:, :3].dot(matrix2.T) + shifts2

    if 'xy_shift_range' in pc2_args and pc2_args['xy_shift_range'] is not None:
        xy_shifts2 = np.random.uniform(-pc2_args['xy_shift_range'],
                                       pc2_args['xy_shift_range'],
                                       (1, 2)).astype(np.float32)
        pc2[:, 0] = pc2[:, 0] + xy_shifts2[0, 0]
        pc2[:, 2] = pc2[:, 2] + xy_shifts2[0, 1]

    return pc2


def do_pc_jitter(pc, jitter_sigma, jitter_clip):
    jitter2 = np.clip(jitter_sigma * np.random.randn(pc.shape[0], 3),
                      -jitter_clip, jitter_clip).astype(np.float32)
    pc[:, :3] += jitter2
    return pc
...
class ProcessData(object):
    def __init__(self, data_process_args, aug_pc2_args, num_points, allow_less_points):
